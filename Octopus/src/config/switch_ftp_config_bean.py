# switch_ftp_config_bean.py
"""
Switch FTP 配置 Bean 模块

该模块定义了 SwitchFtpConfigBean 类，用于配置通过 FTP 传输的相关参数。
"""
...
class SwitchFtpConfigBean:
    """
    Switch FTP 参数配置 Bean。
    ...
    def __init__(self):
        """
        使用默认值初始化 SwitchFtpConfigBean 实例。
        """
        # 是否启用 FTP 传输
    ...
    def add_ftp(self, ftp_config: 'FtpConfigBean') -> None:
        ...
        该方法用于设置 FTP 配置对象，与 set_ftp 方法功能相同。
    ...
    def __str__(self) -> str:
        ...
        Returns:
            str: 对象的字符串表示形式。
        """
        return (f"SwitchFtpConfigBean(is_ftp={self._is_ftp}, "
...
if __name__ == "__main__":
    # 创建 SwitchFtpConfigBean 实例
    switch_ftp_config = SwitchFtpConfigBean()
    
    # 设置属性
    switch_ftp_config.is_ftp = True
    switch_ftp_config.limited_size = 1024 * 1024  # 1MB
    
    # 输出配置信息
    print("Switch FTP 配置:")
    print(f"启用FTP: {switch_ftp_config.is_ftp}")
    print(f"大小限制: {switch_ftp_config.limited_size} 字节")
    print(f"FTP配置: {switch_ftp_config.ftp}")
    
    # 测试字符串表示
    print("\n字符串表示:")
    print(str(switch_ftp_config))
    print(repr(switch_ftp_config))